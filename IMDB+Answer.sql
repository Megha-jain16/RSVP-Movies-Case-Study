...
SELECT id, title, avg_rating, genre,
CASE 
...
ON g.movie_id = m.id
WHERE genre IN ("Drama", "Action", "Comedy") AND worlwide_gross_income IS NOT NULL
...
Count(id) AS movie_count from movie m 
INNER JOIN ratings r
on m.id = r.movie_id
...
SELECT name AS actress_name,
        ...
        Round(Sum(avg_rating*total_votes)/Sum(total_votes),2) as actress_avg_rating,
        DENSE_RANK() OVER (ORDER BY COUNT(id) DESC) AS actress_rank
	FROM role_mapping AS RM INNER JOIN names AS n
ON n.id = RM.name_id INNER JOIN ratings AS r
ON RM.movie_id = r.movie_id INNER JOIN genre AS g
